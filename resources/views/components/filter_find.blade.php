...
<div class='finder-filter'>
    <form method="post" action="{{ route('app::journeys') }}" class='finderTwo__form'>
        ...
        <select name="city" class="find_select_filter form_custom-style">
            <option value="" selected>Выберите город</option>
            @foreach($cities as $city)
                <option value="{{ $city->id }}">{{ $city->title }}</option>
            @endforeach
        </select>
        <select name="transport" class="find_select_filter form_custom-style">
            <option value="" selected>Выберите транспорт</option>
            @foreach($transports as $transport)
                <option value="{{ $transport->id }}">{{ $transport->title }}</option>
            @endforeach
        </select>
        <select name="complexity" class="find_select_filter form_custom-style">
